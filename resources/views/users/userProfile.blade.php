                ...
                <div class="container-fluid py-2">
                    @auth
                        ...
                        <div class="dropdown float-end">
                            <button class="btn btn-secondary btn-circle rounded-circle bg-opacity-10" type="button"
                                id="dropdownMenuButton2" data-bs-toggle="dropdown" aria-expanded="false">
                                <i class="fas fa-ellipsis-h fa-lg"></i>
                            </button>
                            <ul class="dropdown-menu dropdown-menu-dark" aria-labelledby="dropdownMenuButton2">
                                @if (Auth::user()->username == $username)
                                    <li><a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#UpdateAvatar">تغيير
                                            الصورة الرمزية</a></li>
                                    <li><a class="dropdown-item"
                                            href="{{ route('AccountUpdate', $userdetales->username) }}">تعديل البيانات
                                            الشخصيه</a></li>
                                @endif
                    ...
                    <div class="card-body">
                        <img src="{{ url('images', $userdetales->avatar_path) }}" alt="Bootstrap" width="60" height="60"
                                ...
                                @if (in_array(Auth::user()->id, $getFollowers))
                                    <div class="gap-1 justify-content-md-end">
                                        <form action="{{ route('FollowUnfollow', $username) }}" method="POST">
                                            ...
                                            <button type="submit" class="btn btn-success btn-sm me-md-2">Unfollow</button>
                                        </form>
                                    </div>
                                @else
                                    <div class="gap-1 justify-content-md-end">
                                        <form action="{{ route('FollowUnfollow', $username) }}" method="POST">
                                            ...
                                            <button type="submit" class="btn btn-primary btn-sm me-md-2">Follow</button>
                                        </form>
                                    </div>
                                @endif
                            @endif
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content rounded-6 shadow">
                    ...
                    <div class="modal-body py-0">
                        <form action="{{ route('AvatarUpdate', $userdetales->username) }}" method="POST"
                            enctype="multipart/form-data">
                            @csrf
                            @method('POST')
                            <img id="blah" alt="your image" src="{{ url('images', $userdetales->avatar_path) }}" width="60"
                                height="60" class="rounded border border-white" />
